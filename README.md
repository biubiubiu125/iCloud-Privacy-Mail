...
- 邮箱池分类：邮箱池支持按导出状态、使用状态（未使用/已使用/失败/停用）、API 开关和远端核对状态筛选；列表、全选筛选结果、导出和批量操作共用同一套条件。
- 邮箱池批量操作：勾选后可导出/复制选中、改状态、启用或停用 API、绑定账号、同步邮件，以及删除本地记录或同步删除 iCloud 隐私邮箱本体。停用后可再启用。
...
```http
GET /api/v1/mailboxes/{email}/code?key=<mailbox_api_token>&wait_ms=12000
```

### 按邮箱 ID 取码

```http
GET /api/mailboxes/{id}/code?key=<mailbox_api_token>&keyword=OpenAI
```
...
| 参数 | 说明 |
| --- | --- |
| `Authorization` / `key` | 面板复制、导出和领取的 URL 自带 `?key=<mailbox_token>`。也可用 `Authorization: Bearer` 或 `X-API-Key`；全局 `api_key` 仍只能放请求头，不能进 URL |
...
- 邮箱池支持按导出状态、使用状态、API 开关和远端核对状态分类显示；导出邮箱 API 后会记录导出时间并显示在邮箱行上。
- 直接导出会继承当前 TAB、账号范围、首页搜索词、导出状态、使用状态、API 开关和远端核对筛选；批量导出只处理选中的邮箱，未选择时不会误导出全量。
- 邮箱池支持单选、当前页全选和筛选结果全选。
- 删除邮箱时可以选择只删除本地记录，或先删除 iCloud 的隐私邮箱本体，远端删除失败时会保留本地记录并返回失败明细。邮箱池会在标题下显示删除进度和结果；只删本地时不再被账号同步锁挡住。
...
- 复制/导出的完整取码 URL 含该邮箱 `api_token`，应按密钥保存；token 泄露后仍可能读取该邮箱验证码。全局 `api_key` 不要放进 URL。
